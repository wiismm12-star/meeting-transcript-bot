...
- [x] 本機 Web 支援逐字稿、會議摘要與會議筆記分頁
- [x] 本機 Web 支援上傳單一錄音檔並建立會議逐字稿
- [x] 建立僅本機使用的 Web 校稿介面（綁定 127.0.0.1，暫不提供登入）
- [x] 支援逐段播放音檔與逐字稿對齊
- [x] 支援逐字稿段落手動編輯
- [x] 支援本機 Web 主講人名稱批次修改
- [x] 本機 Web 合併同一人的講者，直接改寫逐字稿名稱並移除原 Speaker 標籤
- [x] 本機 Web 支援內嵌修改會議名稱與逐字稿段落，並即時同步主講人別名
- [x] 本機 Web 支援 TXT／Word 逐字稿下載
- [x] 本機 Web 支援音訊播放、點擊逐段時間戳跳轉與 10 秒快轉／倒轉
- [x] 保留同一主講人的原始時間片段，並將超長無標點發言切成易讀的 1–2 行區塊
- [x] 潤稿回寫依主講人段落順序對齊，並在儲存前再次切開超長文字列
- [x] 重用相同音檔與開啟舊會議時，自動修復超長時間軸段落
- [x] 重用相同音檔前驗證時間軸覆蓋率，略過不完整逐字稿
- [x] Windows 轉錄期間的 ffmpeg／ffprobe 背景程序不顯示主控台視窗
- [x] 最近會議卡片僅由「開啟工作台」連結導向，不可點擊卡片開啟
- [x] Web 服務偵測原始碼變更後自動無視窗重啟
- [x] 本機 Web 會議摘要使用本機模型產生標題、概覽與重點條列，並快取於資料庫
- [x] 會議摘要依錄音時長設定閱讀預算（每 10 分鐘錄音約 1 分鐘閱讀，最多 10 分鐘）
- [x] 本機 Web 首頁支援多選、全選與批次刪除會議及本機工作檔
- [x] 修正 Web 刪除會議時，同步移除已缺少音檔的殘留工作資料夾
- [x] 本機 Web 工作台左側顯示所有會議紀錄，可直接切換目前會議
- [x] 上傳或轉錄失敗時自動清理未完成的本機會議資料
- [x] 每次建立新轉錄前清理已失去工作程序的殘留未完成會議與工作檔
- [x] 完成 LINE Bot 公開 HTTPS webhook 實測
- [x] 將 LINE 音檔接入背景轉錄工作佇列
...
- [x] 以同一批台灣中文音檔比較 Deepgram 與 Gladia 的辨識結果
- [x] 以同一批台灣中文音檔比較 Deepgram 與本機 Whisper large-v3 的辨識結果
